...
class DataCollatorForSeq2Seq:
    ...
    def __call__(self, features, return_tensors=None):
        ...
        if labels is not None:
            ...
            for feature in features:
                ...
                if isinstance(feature["labels"], list):
                    ...
                    feature["original_labels"] = (
                        feature["original_labels"] + remainder if padding_side == "right" else remainder + feature["original_labels"]
                    )
                elif padding_side == "right":
                    feature["labels"] = np.concatenate([feature["labels"], remainder]).astype(np.int64)
                    feature["original_labels"] = np.concatenate([feature["original_labels"], remainder]).astype(np.int64)
                else:
                    feature["labels"] = np.concatenate([remainder, feature["labels"]]).astype(np.int64)
                    feature["original_labels"] = np.concatenate([remainder, feature["original_labels"]]).astype(np.int64)
        ...
        if (
            ...
            decoder_input_ids = self.model.prepare_decoder_input_ids_from_labels(labels=features["original_labels"])
            features["decoder_input_ids"] = decoder_input_ids

        features.pop("original_labels")
        return features
